...
let BWDropBoxDefaultColor: UIColor = UIColor.lightGray
let BWDropBoxTitleColor: UIColor = UIColor.black
let BWDropBoxListBGColor: UIColor = UIColor.white
let BWDropBoxListTitleColor: UIColor = UIColor.black
let BWDropBoxListCellHeight: CGFloat = 35.0
let BWDropBoxBackgroundAlpha: CGFloat = 0.3
let BWDropBoxTitleMargin: CGFloat = 15
let BWDropBoxTitleFontSize: CGFloat = 14
let BWDropBoxTitleHoldFontSize: CGFloat = 13
let BWDropBoxListFontSize: CGFloat = 13
let BWDropBoxAnimateTime: Double = 0.3

// 下拉选择弹窗
class BWDropBoxView: UIView {
    ...
    var listTableViewX:CGFloat!
    var listTableViewY:CGFloat!
    
    init(parentVC: UIView, title: String, items: [String], frame: CGRect) {
        ...
        self.boxTitle.font = UIFont.systemFont(ofSize: BWDropBoxTitleHoldFontSize)
        self.boxTitle.textAlignment = .left
        self.boxTitle.textColor = BWDropBoxDefaultColor
        ...
        let resizedImage = image?.setImageSize(CGSize(width: 25, height: 25))
        self.boxArrow = UIImageView(image: resizedImage)
        self.boxButton.addSubview(self.boxArrow)
        
        let parentFrame = parentVC.frame
        ...
        self.listTableViewX = frame.origin.x
        self.listTableViewY = frame.origin.y
        
        self.boxWrapperView.addSubview(self.backgroundView)
        self.boxWrapperView.addSubview(self.listTableView)
        parentVC.addSubview(self.boxWrapperView)
    ...
    override func layoutSubviews() {
        ...
        self.boxTitle.frame.origin = CGPoint(x: BWDropBoxTitleMargin, y: 0)
        ...
        self.listTableView.frame.origin = CGPoint(x: listTableViewX, y: listTableViewY + self.frame.size.height)
        self.listTableView.frame.size = CGSize(width: self.frame.size.width, height: CGFloat(self.items.count) * BWDropBoxListCellHeight)
    ...
    fileprivate func showBoxList() {
        ...
        UIView.animate(
            withDuration: BWDropBoxAnimateTime,
            delay: 0,
            ...
            animations: {
                if self.isHightWhenShowList == true {
                    self.backgroundView.alpha = BWDropBoxBackgroundAlpha
                }
                self.listTableView.frame.size.height = CGFloat(self.items.count) * BWDropBoxListCellHeight
        }, completion: { _ in
    ...
    fileprivate func didSelectItem(_ row: Int) {
        if currentIndex == -1 {
            self.boxTitle.textColor = BWDropBoxTitleColor
            self.boxTitle.font = UIFont.systemFont(ofSize: BWDropBoxTitleFontSize)
        }
...
extension BWDropBoxView: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return BWDropBoxListCellHeight
    }
}

extension BWDropBoxView: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = BWDropBoxTableViewCell(reuseIdentifier: "Cell")
...
class BWDropBoxTableViewCell: UITableViewCell {
    ...
    init(reuseIdentifier: String?) {
        ...
        self.backgroundColor = BWDropBoxListBGColor
        self.textLabel!.textColor = BWDropBoxListTitleColor
        self.textLabel!.font = UIFont.systemFont(ofSize: BWDropBoxListFontSize)
        self.textLabel!.textAlignment = .left
        self.textLabel!.frame.origin.x = BWDropBoxTitleMargin
